function [NHO,E] = kfoldo(trainInputs,trainTargets,NH,kFold,numIter)
%KFOLDO Do ordinal k-Fold cross-validation.
%
%  Syntax
%
%    kfoldo(trainInputs,trainTargets,NHmax,kFold,numIter)
%
%  Description
%
%    KFOLDO(trainInputs,trainTargets,NHmax,kFold,numIter) takes,
...
for i=NH
	...
	for k=1:kFold
		...
		for j=1:numIter
			...
			net = newoff(trainX,trainY,i,'tansig');
			net.trainParam.showWindow = false; % don't show training interface

			% training the net
			net = otrain(net,trainX,trainYNew);
