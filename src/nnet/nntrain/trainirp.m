function [net,tr] = trainirp(net,tr,trainV,valV,testV,varargin)
%TRAINRP iRPROP+ backpropagation.
%
%  Syntax
%  
%    [net,tr,Ac,El] = trainirp(net,tr,trainV,valV,testV)
%    info = trainirp('info')
%
%  Description
%
%    TRAINIRP is a network training function that updates weight and
%    bias values according to the resilient backpropagation algorithm
%     (iRPROP+).
%
%    TRAINIRP(NET,TR,TRAINV,VALV,TESTV) takes these inputs,
%      NET - Neural network.
...
%
%    TRAINIRP('info') returns useful information about this function.
%
%  Network Use
%
%    You can create a standard network that uses TRAINIRP with
%    NEWFF, NEWCF, or NEWELM.
%
%    To prepare a custom network to be trained with TRAINIRP:
%    1) Set NET.trainFcn to 'trainirp'.
%       This will set NET.trainParam to TRAINIRP's default parameters.
%    2) Set NET.trainParam properties to desired values.
%
%    In either case, calling TRAIN with the resulting network will
%    train the network with TRAINIRP.
%
%  Examples
%
...
%    neurons, and the second layer has one LOGSIG neuron.  The TRAINIRP
%    network training function is to be used.
%
%      % Create and Test a Network
%      net = newff([0 5],[2 1],{'tansig','logsig'},'trainirp');
%      a = sim(net,p)
%
...
%  Algorithm
%
%    TRAINIRP can train any network as long as its weight, net input,
...
%
%  See also NEWFF, NEWCF, TRAINRP, TRAINGDM, TRAINGDA, TRAINGDX, TRAINLM,
...
% Updated by Pérula Martínez, Raúl
% Copyright 2011
% $Revision: 1.0 $ $Date: 2011/mm/dd hh:mm:ss $
...
for epoch=0:epochs
  ...
  X = X+(ggX>0 | ggX==0).*dX - (ggX<0 & perf>perf_old).*dX;
